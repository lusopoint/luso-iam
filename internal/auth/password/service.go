...
// generic "login failed" message on the login page
var (
...
// TODO: maybe we could make this a configuration?
const (
	maxFailedAttempts = 5
...
// Authenticate returns the user iff email + password match a valid credential
// the exact error returned indicates the failure mode but
...
func (s *Service) Authenticate(ctx context.Context, email, password string) (*postgres.User, error) {
	...
	// Successful login failures here are logged but don't fail the login
...
// recordFailure bumps the failure counter and, on threshold, sets a lockout window
// errors are logged but not returned failure to record should not affect the user-visible login outcome
...
// paths so an attacker can't distinguish registered emails by request timing

	...
	return []EnvGroup{
		{
			...
			Vars: []EnvVar{
				...
				{Name: "AUTO_MIGRATE", Default: "true", Desc: "Applies any pending database migrations on start.", When: "Keep it true for a single-instance deployment. Set it false when a separate step runs migrations, or when you run more than one instance and want one controlled migration."},

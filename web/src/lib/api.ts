...
export const api = {
  ...
  updateUser: (
    ...
    body: Partial<
      Pick<
        AdminUser,
        'email' | 'username' | 'display_name' | 'status' | 'is_admin'
      >
    >,
  ) => request<AdminUser>(`/users/${id}`, { method: 'PATCH', body }),
  lockUser: (id: string) =>
    request<AdminUser>(`/users/${id}/lock`, { method: 'POST' }),
  unlockUser: (id: string) =>
    request<AdminUser>(`/users/${id}/unlock`, { method: 'POST' }),
  ...
  unlinkUserFederation: (userId: string, linkId: string) =>
    request<void>(`/users/${userId}/federation/${linkId}`, {
      method: 'DELETE',
    }),

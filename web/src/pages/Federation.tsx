      ...
      {error ? (
        ...
        <>
          ...
          <Alert variant="info" title="About this page" className="mt-8">
            <p>
              Provider configuration lives in environment variables, not the
              database. The list above reflects what the server detected at
              boot. Credentials (client ID and secret) are intentionally not
              shown, they don't need to be visible here to verify the
              integration works.
            </p>
            <p className="mt-2">
              To add or change a provider: set the matching env vars (e.g.{' '}
              <code className="font-mono">GOOGLE_CLIENT_ID</code>,{' '}
              <code className="font-mono">GOOGLE_CLIENT_SECRET</code>), register
              the redirect URI shown above in the provider's console, and
              restart the server.
            </p>
          </Alert>
        </>

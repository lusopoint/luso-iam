...
const UserDetail = () => {
  ...
  return (
    <>
      <PageHeader
        breadcrumbs={[{ label: "Users", href: "/users" }, { label: user.email || user.username || "User" }]}
        ...
        <Card noHover>
          ...
          <CardContent>
            <dl className="grid grid-cols-[9rem_minmax(0,1fr)] gap-y-3 text-sm">
              ...
              <Field label="Admin">
                {user.is_admin ? <Badge status="in_progress" label="admin" /> : "No"}
              </Field>
        ...
        <Card noHover>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {user.status === "active" ? (
                <Button variant="outline" size="sm" disabled={Boolean(busy)} onClick={() => run("Lock", () => api.lockUser(id), setUser)}>
                  Lock account
                </Button>
              ) : (
                <Button variant="outline" size="sm" disabled={Boolean(busy)} onClick={() => run("Unlock", () => api.unlockUser(id), setUser)}>
                  Unlock account
                </Button>
              )}
                ...
                <TableRow key={s.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-on-surface-variant">{s.amr.join(", ")}</span>
                    </div>
                  </TableCell>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-on-surface-variant">
                      <span className="font-mono">
                        sub: {link.sub.length > 30 ? link.sub.slice(0, 27) + "..." : link.sub}
                      </span>
                      {" - "}
